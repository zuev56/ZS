...
CREATE OR REPLACE FUNCTION vk.sf_get_not_active_users(
    userIdsStr text,
    offlineHours int)
    RETURNS text
    LANGUAGE 'plpgsql'
...
DECLARE
    _userIds int[];
    _activeUserIds int[];
    _notActiveUserIds int[];
    _result text = null;
BEGIN
    _userIds = string_to_array(userIdsStr, ',');
    --RAISE NOTICE '1. _userIds: %', _userIds;
   
    SELECT INTO _activeUserIds array_agg(DISTINCT user_id)
    FROM vk.activity_log
    WHERE insert_date > now() - (offlineHours || ' hours')::interval
      AND user_id = ANY(_userIds);
      --AND is_online = true; Если есть запись в журнале за последние, значит пользователь точно был онлайн
    --RAISE NOTICE '3. _activeUserIds: %', _activeUserIds;

    SELECT array(SELECT unnest(_userIds) EXCEPT SELECT unnest(_activeUserIds)) INTO _notActiveUserIds;
    --RAISE NOTICE '4. _notActiveUserIds: %', _notActiveUserIds;

    -- Формирование сообщения "<UserName> is not active for <_exactOfflineHours> hours"
    IF (cardinality(_notActiveUserIds) > 0)
    THEN
        ...
        FROM vk.users as u
        WHERE u.id = ANY(_notActiveUserIds);
    END IF;

    RETURN _result;
END;
$BODY$;
ALTER FUNCTION vk.sf_get_not_active_users(text, integer)
    OWNER TO postgres;

--select vk.sf_get_not_active_users('51823577, 8790237, 24344351', 24);
